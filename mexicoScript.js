...
const raceStartTime = 3890;
const raceStartUTC = new Date('2023-01-14T20:03:30.000Z');
...
myVideo.addEventListener('play', () => {
  ...
  myVideo.addEventListener('timeupdate', () => {
    ...
    const postsInCurrentTimeframe = posts.filter(post => {
      ...
      return postTime >= currentSecond && postTime < currentSecond + 60;
    });
...
fetch('mexicoTwitter.json')
  ...
  .then(data => {
    ...
    const counts = {};
    const startDate = new Date('2023-01-14T00:00:00.000Z');
  ...
  .then(data => {
    ...
    for (const fiveMinWindow in counts) {
      if (counts[fiveMinWindow].count > 4) {
